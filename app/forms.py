...
from app.constants import DISP_STATUS, SECRET_STATUS
...
def DisplaySecretEditForm(obj=None, **kwargs):
    class DisplaySecretEditFormImpl(FlaskForm):
        name = StringField("Secret Name", validators=[DataRequired()], description="A name for your reference")
        if not obj:
            key = StringField("Secret Key", validators=[Optional()], description="If you do not enter a custom secret key, one will be autogenerated.")
        form_status = SelectField("Status", choices=[(k, v) for k, v in SECRET_STATUS.items()], validators=[DataRequired()])
        if obj:
    ...
    return DisplaySecretEditFormImpl(obj=obj, **kwargs)
