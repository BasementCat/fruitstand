...
def weather_suffix(units, what='temp'):
    ...
    return suffixes.get(what, {}).get(units, '???')


@jfilter()
def weather_convert(value, units, what, d=2):
    conversion = {
        'pressure': {
            'standard': lambda v: v,
        ...
        'distance': {
            'standard': lambda v: v,
            'imperial': lambda v: v / 1609.34,
            'metric': lambda v: v / 1000,
        },
    }
    value = conversion.get(what, {}).get(units, lambda v: v)(value)
    value = fixed(value, d=d)
    suffix = weather_suffix(units, what=what)
